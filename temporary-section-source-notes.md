...
The official host does not grant browser CORS access to this PDF. NextLecture therefore retrieves and validates the document server-side with bounded byte ranges, caches the binary payload for 24 hours, and supplies it through a same-origin `/api/syllabus.pdf` endpoint. The client turns that binary response into Gemini PDF input only after a student starts a request. A student’s optional device-local Gemini key is not sent to NextLecture; it remains in browser local storage and is sent straight to Gemini in the `x-goog-api-key` request header.

Production verification for commit `3e7edd3` succeeded on Vercel. The public `/syllabus` route rendered its Gemini settings, local history, example prompts, and official-source state. A direct same-origin request to `/api/syllabus.pdf` returned `200`, `application/pdf`, and the expected 1,110,068-byte payload, with the server-supplied retrieval timestamp. No Gemini key was needed or sent during this verification.

## Gemini server fallback configuration

When no device-local key is saved, the browser calls NextLecture’s `/api/syllabus/stream` endpoint. The server uses the `GEMINI_API_KEY` environment variable only for that fallback path, so the value never reaches the browser. A device-local key always takes priority and can be cleared in **AI settings** by choosing **Use server fallback**. To enable the fallback in Vercel, open the `nextlecture` project, select **Settings → Environment Variables**, add `GEMINI_API_KEY` with the Gemini API key value for the required environments (at minimum **Production**), then redeploy. Until the variable is set, the stream endpoint deliberately returns `503` with a configuration message rather than silently attempting an unauthenticated Gemini request.
