...
export function StudentProfileSetup({ branch, onSaved, onDismiss }: { branch: string; onSaved: (profile: StudentProfile) => void; onDismiss: () => void }) {
  ...
  const profileQuery = trpc.temporarySections.profile.useQuery(
    { branch, registrationNumber: registrationNumber ?? "000000" },
    { enabled: Boolean(registrationNumber), retry: 0 },
  );
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">Search the official GNDEC 2026 temporary-section document by name. Your selected profile is saved only on this device.</p>
            </div>
          </div>
      ...
      <div className="p-5 sm:p-7">
        {!manualMode ? (
          <>
            ...
            {matchesQuery.data && <div className="mt-4 rounded-xl border border-border p-2">
              ...
              {matchesQuery.data.matches.length ? <div className="max-h-64 overflow-y-auto"><div className="grid gap-1">
                {matchesQuery.data.matches.map(match => <button key={match.registrationNumber} type="button" disabled={Boolean(registrationNumber)} onClick={() => setRegistrationNumber(match.registrationNumber)} className="flex min-h-13 items-center justify-between gap-3 rounded-lg px-3 py-2 text-left transition hover:bg-teal-50 disabled:opacity-60 dark:hover:bg-teal-950/30">
                  <span><span className="block font-semibold">{match.candidateName}</span><span className="mt-0.5 block text-xs text-muted-foreground">{match.temporarySubsection} · Roll {match.rollNumber}</span></span>
                  <span className="shrink-0 text-sm font-bold text-teal-700 dark:text-teal-300">({match.registrationNumber})</span>
                </button>)}
            ...
            {registrationNumber && <div className="mt-4 flex items-center gap-2 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm text-teal-900 dark:border-teal-900/60 dark:bg-teal-950/25 dark:text-teal-100"><LoaderCircle className="h-4 w-4 animate-spin" />Saving the selected official profile…</div>}
          ...
          <form onSubmit={saveManualProfile} className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2"><p className="eyebrow">MANUAL PROFILE</p><h3 className="mt-1 font-display text-xl font-semibold tracking-[-0.04em]">Enter your temporary-section details</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">Use this when the official document cannot be loaded or your name is not listed. You can update it later.</p></div>
            {([['candidateName', 'Full name', 'Your name'], ['registrationNumber', 'Registration number', 'For example 26013850'], ['rollNumber', 'Roll number / Sr. no.', 'Optional'], ['temporarySection', 'Temporary section', `For example ${branch}B`], ['temporarySubsection', 'Temporary subsection', `For example ${branch}B2`], ['mentorName', 'Mentor name', 'Optional']] as const).map(([field, label, placeholder]) => <label key={field} className="block text-sm font-semibold">{label}<input value={manual[field]} onChange={event => setManual(current => ({ ...current, [field]: event.target.value }))} required={field === "candidateName" || field === "registrationNumber"} placeholder={placeholder} className="mt-2 h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-normal outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/10" /></label>)}
