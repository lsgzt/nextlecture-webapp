...
function ScheduleCard({ lecture, now }: { lecture: NonNullable<LocalTimetable>["timetable"]["lectures"][number]; now: Date }) {
  ...
  const typeLabel = formatLectureType(lecture.lectureType);
  return (
    <article className={`nl-enter relative rounded-2xl border p-4 transition ${status === "current" ? "border-teal-500 bg-teal-50/75 shadow-md shadow-teal-950/5 dark:border-teal-600 dark:bg-teal-950/35" : status === "past" ? "border-border bg-card/45 opacity-55" : "border-border bg-card"}`}>
      <div className="flex gap-3">
        <p className="w-14 shrink-0 pt-0.5 text-sm font-bold tracking-[-0.02em] text-muted-foreground"><CompactTime time={lecture.startTime} /></p>
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h3 className="font-semibold leading-5 text-foreground">{lecture.subject}</h3>
              {typeLabel && <p className="mt-1 text-xs font-semibold tracking-[0.04em] text-teal-700 dark:text-teal-300">{typeLabel}</p>}
            </div>
            <span className={`shrink-0 rounded-full px-2 py-1 text-[0.63rem] font-bold tracking-wide ${status === "current" ? "bg-teal-700 text-white" : status === "past" ? "bg-muted text-muted-foreground" : "bg-teal-50 text-teal-700 dark:bg-teal-950/50 dark:text-teal-300"}`}>{stateCopy}</span>
          </div>
          <p className="mt-1.5 text-sm text-muted-foreground"><CompactRange startTime={lecture.startTime} endTime={lecture.endTime} /></p>
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5 text-sm text-muted-foreground">
            {lecture.venue && <span className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 text-teal-600 dark:text-teal-400" />{lecture.venue}</span>}
            {lecture.teacher && <span className="inline-flex items-center gap-1.5"><UserRound className="h-3.5 w-3.5 text-teal-600 dark:text-teal-400" />{lecture.teacher}</span>}
          </div>
        </div>
      </div>
    </article>
...
export default function TimetableApp() {
  ...
  const tomorrow = useMemo(() => { const value = new Date(now); value.setDate(value.getDate() + 1); return value; }, [now]);
  const tomorrowDay = useMemo(() => weekdayForDate(tomorrow), [tomorrow]);
  const tomorrowLectures = useMemo(() => getTomorrowLectures(data?.timetable ?? null, now), [data?.timetable, now]);
  ...
  const tomorrowTimeline = useMemo<TimelineItem[]>(() => (data?.timetable.timeSlots ?? []).flatMap<TimelineItem>(slot => {
    const scheduled = tomorrowLectures.find(lecture => lecture.startTime === slot);
    if (scheduled) return [{ kind: "lecture" as const, lecture: scheduled }];
    const coveredByLongLecture = tomorrowLectures.some(lecture => timeToMinutes(lecture.startTime) < timeToMinutes(slot) && timeToMinutes(lecture.endTime) > timeToMinutes(slot));
    return coveredByLongLecture ? [] : [{ kind: "free" as const, startTime: slot }];
  }), [data?.timetable.timeSlots, tomorrowLectures]);
  ...
  return <div className="min-h-screen bg-[#f7f8f6] text-foreground dark:bg-[#101917]"><header className="app-topbar"><div className="container flex h-17 items-center justify-between"><div className="flex items-center gap-3"><Link href="/" className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-card text-muted-foreground transition hover:text-teal-700 dark:hover:text-teal-300" aria-label="Back to NextLecture home"><ArrowLeft className="h-4 w-4" /></Link><img src={BRAND_LOGO_URL} alt="NextLecture timetable logo" width={32} height={32} className="h-8 w-8 shrink-0 rounded-xl object-cover shadow-sm shadow-teal-950/25" /><div><Link href="/" className="font-display text-lg font-semibold tracking-[-0.04em]">NextLecture</Link><p className="text-[0.65rem] font-semibold tracking-[0.13em] text-teal-700 dark:text-teal-300">GNDEC TIMETABLE</p></div></div><div className="flex items-center gap-1.5 sm:gap-2"><Link href="/papers" className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-card text-muted-foreground transition hover:border-teal-200 hover:bg-teal-50 hover:text-teal-700 dark:hover:border-teal-900 dark:hover:bg-teal-950/35 dark:hover:text-teal-300 sm:hidden" aria-label="Open Previous Year Papers"><FileText className="h-4 w-4" /></Link><Link href="/papers" className="hidden rounded-lg px-3 py-2 text-sm font-bold text-muted-foreground transition hover:bg-teal-50 hover:text-teal-700 dark:hover:bg-teal-950/40 dark:hover:text-teal-300 sm:inline-flex">Previous papers</Link><Link href="/syllabus" className="grid h-9 w-9 place-items-center rounded-xl border border-teal-200 bg-teal-50 text-teal-700 transition hover:bg-teal-100 dark:border-teal-900 dark:bg-teal-950/35 dark:text-teal-300 sm:hidden" aria-label="Open Syllabus AI"><BookOpenText className="h-4 w-4" /></Link><Link href="/syllabus" className="hidden rounded-lg px-3 py-2 text-sm font-bold text-teal-700 transition hover:bg-teal-50 hover:text-teal-950 dark:text-teal-300 dark:hover:bg-teal-950/40 dark:hover:text-white sm:inline-flex">Syllabus AI</Link><ThemeToggle /></div></div></header><main className="container max-w-5xl pb-14 pt-4 sm:pt-5">
    ...
    <AnnouncementBanner className="mb-3" />
      {data && <section className="nl-enter mb-4 rounded-[1.5rem] bg-teal-700 p-4 text-white shadow-xl shadow-teal-950/15 sm:mb-5 sm:rounded-[1.75rem] sm:p-5"><div className="flex items-start justify-between gap-3"><p className="text-[0.68rem] font-bold tracking-[0.15em] text-teal-100">{next?.phase === "current" ? "HAPPENING NOW" : "NEXT LECTURE"}</p><span className="rounded-full bg-white/13 px-2.5 py-1 text-xs font-semibold text-teal-50">{next ? (next.phase === "current" ? `Ends in ${humanizeDuration(Math.max(0, timeToMinutes(next.lecture.endTime) - (now.getHours() * 60 + now.getMinutes())))}` : next.dayOffset === 0 ? `Starts in ${humanizeDuration(Math.max(0, timeToMinutes(next.lecture.startTime) - (now.getHours() * 60 + now.getMinutes())))}` : getDayLabel(next.dayOffset, now)) : "No more lectures"}</span></div>{next ? <><h2 className="mt-3.5 font-display text-[1.65rem] font-semibold leading-none tracking-[-0.055em] sm:text-3xl">{next.lecture.subject}</h2>{formatLectureType(next.lecture.lectureType) && <p className="mt-2 text-sm font-semibold tracking-[0.06em] text-teal-100/90">{formatLectureType(next.lecture.lectureType)}</p>}<p className="mt-3 font-medium text-teal-100">{next.phase === "current" ? "Now" : getDayLabel(next.dayOffset, now)} · <CompactRange startTime={next.lecture.startTime} endTime={next.lecture.endTime} /></p><div className="mt-3.5 grid gap-2 border-t border-white/15 pt-3 sm:grid-cols-2">{next.lecture.venue && <p className="inline-flex items-center gap-2 text-sm font-semibold"><MapPin className="h-4 w-4 text-teal-200" />{next.lecture.venue}</p>}{next.lecture.teacher && <p className="inline-flex items-center gap-2 text-sm font-semibold text-teal-100"><UserRound className="h-4 w-4 text-teal-200" />{next.lecture.teacher}</p>}</div></> : <><h2 className="mt-3.5 font-display text-[1.65rem] font-semibold tracking-[-0.055em] sm:text-3xl">No more lectures today.</h2><p className="mt-3 text-teal-100">Your timetable will be ready when the next class comes around.</p></>}</section>}
    ...
    {selectedGroup && !showPicker && <div className="space-y-6">
      ...
      {studentProfile && profileMatchesSelectedBranch && !showProfileSetup && <section className="mb-3 rounded-2xl border border-border bg-card px-4 py-3 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-teal-50 text-teal-700 dark:bg-teal-950/45 dark:text-teal-300"><UserRound className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate font-semibold">{studentProfile.studentName}</p><p className="mt-0.5 truncate text-sm text-muted-foreground">{getStudentProfileSubtitle(studentProfile)}</p></div></div><div className="flex flex-wrap items-center gap-x-4 gap-y-1"><Link href="/attendance" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-teal-200 bg-teal-50 px-3.5 text-sm font-bold text-teal-800 transition hover:bg-teal-100 dark:border-teal-900 dark:bg-teal-950/35 dark:text-teal-200"><ClipboardCheck className="h-4 w-4" />View attendance</Link><button type="button" aria-expanded={isProfileExpanded} aria-controls="full-student-profile" onClick={() => setIsProfileExpanded(current => !current)} className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-teal-700 underline decoration-teal-400 underline-offset-4 hover:text-teal-950 dark:text-teal-300 dark:hover:text-white"><ChevronDown className={`h-4 w-4 transition-transform duration-300 ease-out ${isProfileExpanded ? "rotate-180" : ""}`} />{isProfileExpanded ? "Hide your full information" : "View your full information"}</button><button type="button" onClick={() => setShowProfileSetup(true)} className="inline-flex min-h-10 items-center gap-2 text-sm font-bold text-teal-700 underline decoration-teal-400 underline-offset-4 hover:text-teal-950 dark:text-teal-300 dark:hover:text-white"><PencilLine className="h-4 w-4" />Update profile</button></div></div><Expandable open={isProfileExpanded} id="full-student-profile"><dl className="mt-4 grid gap-x-5 gap-y-3 border-t border-border pt-4 text-sm sm:grid-cols-2 lg:grid-cols-3">{getStudentProfileDetailFields(studentProfile).map(({ label, value }) => <div key={label} className="min-w-0"><dt className="text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">{label}</dt><dd className="mt-1 break-words font-medium">{value || "Not provided"}</dd></div>)}</dl></Expandable></section>}
      <section className="mb-4 flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-end sm:justify-between sm:gap-4 sm:p-5"><div><button type="button" onClick={() => setShowPicker(true)} className="group inline-flex items-center gap-1 rounded-lg py-0.5 font-display text-xl font-semibold tracking-[-0.05em] text-foreground transition hover:text-teal-700 dark:hover:text-teal-300">{selectedGroup}<ChevronDown className="h-4 w-4 transition group-hover:translate-y-0.5" /></button><h1 className="mt-0.5 font-display text-xl font-semibold tracking-[-0.055em] sm:text-2xl">{greeting}<span aria-hidden="true">.</span></h1><div className="mt-2"><Freshness fetchedAt={data?.fetchedAt ?? localTimetable?.fetchedAt} freshness={data?.freshness ?? "stale"} updateError={data?.updateError} /></div></div><button type="button" onClick={refresh} disabled={refreshMutation.isPending || !isOnline} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-border bg-background px-4 text-sm font-semibold transition hover:border-teal-700/30 hover:text-teal-700 disabled:cursor-not-allowed disabled:opacity-60 dark:hover:text-teal-300"><RefreshCw className={`h-4 w-4 ${refreshMutation.isPending ? "animate-spin" : ""}`} />{refreshMutation.isPending ? "Fetching timetable…" : "Fetch again"}</button></section>
      ...
      {data && <>
        <Link href="/vacant-rooms" className="group flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3.5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-teal-300 hover:bg-teal-50/50 hover:shadow-md active:scale-[0.99] dark:hover:border-teal-800 dark:hover:bg-teal-950/30">
        ...
        <section className="grid gap-5 lg:grid-cols-[1.55fr_1fr]"><div className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-teal-50 text-teal-700 dark:bg-teal-950/45 dark:text-teal-300"><CalendarDays className="h-4 w-4" /></span><div><p className="font-display text-xl font-semibold tracking-[-0.04em]">{scheduleView === "today" ? "Today’s timetable" : scheduleView === "tomorrow" ? "Tomorrow’s timetable" : "This week’s timetable"}</p><p className="text-sm text-muted-foreground">{scheduleView === "today" ? new Intl.DateTimeFormat(undefined, { weekday: "long", month: "short", day: "numeric" }).format(now) : scheduleView === "tomorrow" ? new Intl.DateTimeFormat(undefined, { weekday: "long", month: "short", day: "numeric" }).format(tomorrow) : "Monday to Friday"}</p></div></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setScheduleView("today")} className={`min-h-9 rounded-lg px-3 text-sm font-bold transition ${scheduleView === "today" ? "bg-teal-700 text-white" : "border border-border bg-background text-muted-foreground hover:text-teal-700"}`}>Today</button><button type="button" onClick={() => setScheduleView("tomorrow")} className={`min-h-9 rounded-lg px-3 text-sm font-bold transition ${scheduleView === "tomorrow" ? "bg-teal-700 text-white" : "border border-border bg-background text-muted-foreground hover:text-teal-700"}`}>Tomorrow</button><button type="button" onClick={() => setScheduleView("week")} className={`min-h-9 rounded-lg px-3 text-sm font-bold transition ${scheduleView === "week" ? "bg-teal-700 text-white" : "border border-border bg-background text-muted-foreground hover:text-teal-700"}`}>Full week</button></div></div>{scheduleView !== "week" ? <div className="mt-6 space-y-3">{(scheduleView === "today" ? dayTimeline : tomorrowTimeline).length ? (scheduleView === "today" ? dayTimeline : tomorrowTimeline).map(item => item.kind === "lecture" ? <ScheduleCard key={`${item.lecture.day}-${item.lecture.startTime}-${item.lecture.subject}`} lecture={item.lecture} now={now} /> : <article key={item.startTime} className="nl-enter flex items-center gap-3 rounded-2xl border border-dashed border-border bg-muted/30 p-4"><p className="w-14 shrink-0 text-sm font-bold text-muted-foreground"><CompactTime time={item.startTime} /></p><div><p className="font-semibold text-muted-foreground">Free period</p><p className="mt-1 text-sm text-muted-foreground">No lecture scheduled</p></div></article>) : <div className="rounded-2xl bg-muted/55 px-5 py-9 text-center"><Clock3 className="mx-auto h-5 w-5 text-muted-foreground" /><p className="mt-3 font-semibold">No timetable slots {scheduleView === "today" ? "today" : "tomorrow"}</p><p className="mt-1 text-sm text-muted-foreground">Enjoy the breathing room.</p></div>}</div> : <div className="mt-6 space-y-5">{DAY_ORDER.map(day => { const lectures = getLecturesForDay(data.timetable, day); return <section key={day} className="rounded-2xl border border-border bg-background p-4"><div className="flex items-center justify-between gap-3"><h3 className="font-display text-lg font-semibold tracking-[-0.035em]">{day}</h3><span className="text-xs font-bold text-muted-foreground">{lectures.length} {lectures.length === 1 ? "lecture" : "lectures"}</span></div>{lectures.length ? <div className="mt-3 space-y-2">{lectures.map(lecture => <ScheduleCard key={`${day}-${lecture.startTime}-${lecture.subject}`} lecture={lecture} now={now} />)}</div> : <p className="mt-3 text-sm text-muted-foreground">No scheduled lectures.</p>}</section>; })}</div>}</div><aside className="rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-teal-50 text-teal-700 dark:bg-teal-950/45 dark:text-teal-300"><Route className="h-4 w-4" /></span><div><p className="font-display text-xl font-semibold tracking-[-0.04em]">Your group</p><p className="text-sm text-muted-foreground">Update it anytime</p></div></div><div className="mt-6 space-y-4"><label className="block text-sm font-semibold">Semester / cohort<select value={selectedYear} onChange={event => changeYear(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-medium outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/10">{availableYears.map(year => <option key={year} value={year}>{year.replace(/^Year\s+/i, "")}</option>)}</select></label><label className="block text-sm font-semibold">Branch<select value={selectedBranch} onChange={event => changeBranch(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-medium outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/10">{availableBranches.map(branch => <option key={branch} value={branch}>{branch}</option>)}</select></label><label className="block text-sm font-semibold">Section<select value={selectedGroup} onChange={event => chooseGroup(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-input bg-background px-3 text-sm font-medium outline-none focus:border-teal-600 focus:ring-4 focus:ring-teal-600/10">{availableSections.map(group => <option key={group.code} value={group.code}>{deriveGroupParts(group.code).section} · {group.code}</option>)}</select></label></div><button type="button" onClick={() => setShowPicker(true)} className="mt-6 inline-flex text-sm font-bold text-teal-700 underline decoration-teal-400 underline-offset-4 hover:text-teal-950 dark:text-teal-300 dark:hover:text-white">Search every timetable group</button></aside></section>
      </>}
    </div>}
