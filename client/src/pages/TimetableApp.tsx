...
import { DAY_ORDER, deriveGroupParts, formatLectureType, formatRange, getDayLabel, getLecturesForDay, getNextLecture, getTodayLectures, getTomorrowLectures, humanizeDuration, lectureStatus, timeToMinutes } from "@/lib/timetable-ui";
  ...
  return (
    <article className={`relative rounded-2xl border p-4 transition ${status === "current" ? "border-teal-500 bg-teal-50/75 shadow-md shadow-teal-950/5 dark:border-teal-600 dark:bg-teal-950/35" : status === "past" ? "border-border bg-card/45 opacity-55" : "border-border bg-card"}`}>
      <div className="flex gap-3">
        <p className="w-14 shrink-0 pt-0.5 text-sm font-bold tracking-[-0.02em] text-muted-foreground">{lecture.startTime}</p>
