...
import { AlertCircle, ArrowLeft, BookOpenText, CalendarDays, CheckCircle2, ChevronDown, Clock3, CloudOff, ExternalLink, Info, LoaderCircle, MapPin, PencilLine, RefreshCw, Route, Search, UserRound } from "lucide-react";
...
export default function TimetableApp() {
  ...
  return <div className="min-h-screen bg-[#f7f8f6] text-foreground dark:bg-[#101917]"><header className="app-topbar"><div className="container flex h-17 items-center justify-between"><div className="flex items-center gap-3"><Link href="/" className="grid h-9 w-9 place-items-center rounded-xl border border-border bg-card text-muted-foreground transition hover:text-teal-700 dark:hover:text-teal-300" aria-label="Back to NextLecture home"><ArrowLeft className="h-4 w-4" /></Link><img src={BRAND_LOGO_URL} alt="NextLecture timetable logo" width={32} height={32} className="h-8 w-8 shrink-0 rounded-xl object-cover shadow-sm shadow-teal-950/25" /><div><Link href="/" className="font-display text-lg font-semibold tracking-[-0.04em]">NextLecture</Link><p className="text-[0.65rem] font-semibold tracking-[0.13em] text-teal-700 dark:text-teal-300">GNDEC TIMETABLE</p></div></div><div className="flex items-center gap-2"><Link href="/syllabus" className="grid h-9 w-9 place-items-center rounded-xl border border-teal-200 bg-teal-50 text-teal-700 transition hover:bg-teal-100 dark:border-teal-900 dark:bg-teal-950/35 dark:text-teal-300 sm:hidden" aria-label="Open Syllabus AI"><BookOpenText className="h-4 w-4" /></Link><Link href="/syllabus" className="hidden rounded-lg px-3 py-2 text-sm font-bold text-teal-700 transition hover:bg-teal-50 hover:text-teal-950 dark:text-teal-300 dark:hover:bg-teal-950/40 dark:hover:text-white sm:inline-flex">Syllabus AI</Link><ThemeToggle /></div></div></header><main className="container max-w-5xl pb-14 pt-7 sm:pt-10">
    ...
    {selectedGroup && !showPicker && <div className="space-y-6">
      ...
      {studentProfile && profileMatchesSelectedBranch && !showProfileSetup && <section className="rounded-2xl border border-border bg-card px-5 py-4 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-teal-50 text-teal-700 dark:bg-teal-950/45 dark:text-teal-300"><UserRound className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate font-semibold">{studentProfile.studentName}<span className="ml-2 font-normal text-muted-foreground">(CRN {studentProfile.crn})</span></p><p className="mt-0.5 text-sm text-muted-foreground">{studentProfile.subsection} · {studentProfile.mentorName ?? "Mentor not provided"}{studentProfile.source === "manual" ? " · Manual profile" : ""}</p></div></div><button type="button" onClick={() => setShowProfileSetup(true)} className="inline-flex min-h-10 shrink-0 items-center gap-2 text-sm font-bold text-teal-700 underline decoration-teal-400 underline-offset-4 hover:text-teal-950 dark:text-teal-300 dark:hover:text-white"><PencilLine className="h-4 w-4" />Update profile</button></div><dl className="mt-4 grid gap-x-5 gap-y-3 border-t border-border pt-4 text-sm sm:grid-cols-2 lg:grid-cols-3">{([['CRN', studentProfile.crn], ['Father name', studentProfile.fatherName], ['Mother name', studentProfile.motherName], ['Branch', studentProfile.branch], ['Section', studentProfile.section], ['Subsection', studentProfile.subsection], ['Mentoring group', studentProfile.mentoringGroup], ['Mentor name', studentProfile.mentorName], ['Mentor mobile', studentProfile.mentorMobileNumber], ['Venue', studentProfile.venue]] as const).map(([label, value]) => <div key={label} className="min-w-0"><dt className="text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">{label}</dt><dd className="mt-1 break-words font-medium">{value || "Not provided"}</dd></div>)}</dl></section>}
